...
class Toaster {
    ...
    add (config) {
        ...
        if (_config.title && toastHeader) {
            const title = typeof _config.title === "function"
                ? _config.title.call(this.element, _config)
                : _config.title;
        ...
        if (_config.body) {
            ...
            if (body !== null) {
                Util.append(body,
                    typeof _config.body === "function" ? _config.body.call(this.element, _config) : _config.body);
        ...
        if (!(_config.image || _config.icon))
            newToast.querySelectorAll(".toast-image").forEach((el) => Util.remove(el));
    ...
    addEl (element, config) {
        ...
        if (!this.element.getAttribute("id"))
            this.element.setAttribute("id", `ace-toaster-item-${++this._lastToastId}`);
    ...
    _addToContainer (toast, _config, isNewElement = true) {
        ...
        if (container === null) {
            container =
                Util.append(document.body, `<div class="ace-toaster-container position-${_config.placement}"/>`);
        }
        if (_config.belowNav) {
        ...
        if (dummy === null)
            dummy = Util.append(document.body,
            ...
            window.jQuery(toast)
                ...
                .one("hidden.bs.toast.1",
                    function() {
                        ...
                        $toast.removeClass("hide").parent().addClass("show").collapse("hide").one("hidden.bs.collapse",
                            function() {
                                $toast.toast("dispose");
    ...
    hideAll (placement = null, remove = false, instant = false) {
        ...
        const clearEvent = EventHandler.trigger(document,
            Event.CLEAR,
            { detail: { placement: placement || "all", remove: remove } });

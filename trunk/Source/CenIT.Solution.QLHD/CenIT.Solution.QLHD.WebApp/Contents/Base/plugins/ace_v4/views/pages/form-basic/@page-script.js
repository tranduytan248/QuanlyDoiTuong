jQuery(function($) {
    ...
    $("#tooltip-1").tooltip({
        container: "body",
        template:
            '<div class="tooltip" role="tooltip"><div class="arrow brc-success-m1"></div><div class="bgc-success-m1 tooltip-inner text-dark text-600 text-110 px-2 pb-2"></div></div>'
    });
    ...
    $("#popover-1").popover({
        ...
        placement: "auto",
        template:
            '<div class="popover brc-primary-m2" role="tooltip"><div class="arrow arrow2 brc-white"></div><div class="arrow brc-primary-m1"></div><h3 class="popover-header bgc-primary-l2 border-0 text-115 text-dark-tp3"></h3><div class="popover-body text-grey-d2 text-105"></div></div>'
    });
    ...
    $("#id-textarea-limit2").maxlength({
        ...
        message: function(currentText, maxLength) {
            return '<div class="arrow ' +
                (maxLength == currentText.length ? "brc-danger" : "brc-primary") +
                '" style="left:calc(50% - 0.5rem)"></div>\
              <div class="popover-body">' +
                (Math.max(0, maxLength - currentText.length)) +
                ' characters remaining ...\
              <div class="h-1"></div> max allowed: ' +
                maxLength +
                "!</div>";
        }
    });
    ...
    var slider1 =
        $("#id-slider-1").css({ 'height': "260px", 'z-index': 10 })
    ...
    $(slider5).find(".noUi-handle").addClass("brc-white-tp1 bgc-warning-d2 radius-round slider-bars-none shadow")
        .css("border-width", "8px").end()
    ...
    fileInput.addEventListener("changed.ace.file",
        function(e) {
    ...
    $("#id-daterange-from, #id-daterange-to").on("focus",
        function() {
    ...
    $("#id-popover-timepicker")
        .popover({
            ...
            template:
                '<div class="popover popover-timepicker brc-primary-m3 border-1 radius-1 shadow-sm" role="tooltip"><div class="arrow brc-primary"></div><h3 class="popover-header bgc-primary-l3 brc-primary-l1 text-dark-tp4 text-600 text-center pb-25"></h3><div class="popover-body text-grey-d2 p-4"></div></div>'
        })
        .on("click",
            function() {
                ...
                if (popover.length > 0 && popover.hasClass("show")) {
                    var This = this;
                    $(document).on("click.popover-time",
                        function(ev) {
                            if (popover.get(0).contains(ev.target) ||
                                ev.target == document.getElementById("id-popover-timepicker")) return;
                            $(This).popover("hide");

$(function() {
    ...
    QUnit.test("should provide no conflict",
        function(assert) {
            assert.expect(1);
            assert.strictEqual(typeof $.fn.aceFileInput,
                "undefined",
                "aceFileInput was set back to undefined (org value)");
        });


    QUnit.test("should return jquery collection containing the file input element.",
        function(assert) {
            assert.expect(2);
    ...
    QUnit.test("Should not convert to file input if element is not `input[type=file]`",
        function(assert) {
            assert.expect(1);
    ...
    QUnit.test("check basic passed parameters",
        function(assert) {
            assert.expect(4);
            ...
            assert.strictEqual(container.find(".ace-file-name").text().trim(),
                $placeholderText,
                "Contains the passed `placeholderText`");
            assert.strictEqual(container.find(".ace-file-btn").text().trim(),
                $chooseText,
                "Contains the passed `chooseText`");
    ...
    QUnit.test("should correctly set `accept` attribute when `allowMime` or `allowExt` is specified",
        function(assert) {
            assert.expect(2);
            var $el = $('<input type="file" />').appendTo("#qunit-fixture").aceFileInputNC({
                allowExt: "png|gif",
    ...
    QUnit.test("should disable/enable file input element",
        function(assert) {
            assert.expect(2);
            var $el = $('<input type="file" />').appendTo("#qunit-fixture").aceFileInputNC();
    ...
    QUnit.test("should show/hide loading overlay ",
        function(assert) {
            assert.expect(2);
            var $el = $('<input type="file" />').appendTo("#qunit-fixture").aceFileInputNC();

            $el.aceFileInputNC("startLoading",
                '<i class="overlay-content fa fa-spin fa-spinner text-white fa-2x">IS LOADING</i>');
    ...
    QUnit.test("should show file list ",
        function(assert) {
            assert.expect(4);

            var $el = $('<input type="file" multiple />').appendTo("#qunit-fixture").aceFileInputNC();

            $el.aceFileInputNC("showFileList",
                [{ name: "111.png", type: "image" }, { name: "222.txt", type: "document" }]);
    ...
    QUnit.test("should change default video file icon",
        function(assert) {
            assert.expect(2);
            ...
            assert.equal(container.find("i.fa-file-alt").length,
                1,
                "Shows default `fa-file-alt` icon for document file");
        });


    QUnit.test("show check file list and remove unwanted ones",
        function(assert) {
            assert.expect(4);
    ...
    QUnit.test("should trigger `invalid.ace.file` event",
        function(assert) {
            assert.expect(5);
            ...
            $el.get(0).addEventListener("invalid.ace.file",
                function(e) {
                    var p = e.$_fileErrors;
    ...
    QUnit.test("should return fileinput version",
        function(assert) {
            assert.expect(1);

...
namespace Cores.Base.Apps
{
    ...
    ///
    /// Viec trung ten voi Cores.Sys.Apps.AppController duoc xu ly bang route
    /// rieng trong RouteConfig, chi dinh ro namespace Cores.Base.Apps.
    /// </remarks>
    public class AppController : BaseController
    {
        ...
        private readonly List<string> _listSuperAdmins;
        ...
        {
            var configSuperAdmins = _configCache.GetViaKey(CONFIG_SUPER_ADMIN_PERMIT)?.ConfigValue;
            if (!string.IsNullOrEmpty(configSuperAdmins)) _listSuperAdmins = configSuperAdmins.Split(',').ToList();
        }

        protected override void OnResultExecuted(ResultExecutedContext filterContext)
        {
            var isSuperAdmin = (Request?.IsLocal ?? false) ||
                               (_listSuperAdmins != null && _listSuperAdmins.Contains(User?.UserName));
            filterContext.HttpContext.Items["IsSuperAdmin"] = isSuperAdmin;
        ...
        protected string[] GetPermissionViaUser(string userName)
        {
            ...
            var permissionCache = new SysPermissionCache();
            ...
            lstPermissions.ForEach(p =>
            {
                ...
                {
                    var controllerDescriptor = new ReflectedControllerDescriptor(c);

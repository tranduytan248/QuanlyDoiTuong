...
module.exports = class PageList {
    ...
    _makeSidebarTree() {
        ...
        for (let item of this._sidebarList) {
            ...
            if (this._sidebarID_Map[parentId]) {
                if (!Array.isArray(this._sidebarID_Map[parentId].children))
                    this._sidebarID_Map[parentId].children = []; //create the children array if not already
    ...
    _markSelectedItems(activePageID) {
        ...
        this._breadcrumbsList =
            this._breadcrumbsList.reverse(); //so that it's displayed like this : root > parent > current node , etc
    ...
    getDescription(_id=null) {
        let id = _id ? _id : this._lastActiveID;
        return this._sidebarID_Map[id]
            ? (this._sidebarID_Map[id].description ? this._sidebarID_Map[id].description : "")
            : "";
    }

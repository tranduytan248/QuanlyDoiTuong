...
class Sticky {
    ...
    _init () {
        ...
        setTimeout(() => {
                if (observedChild.getBoundingClientRect().bottom < 0) {
                    ...
                    const evt = new window.CustomEvent("sticky-change",
                        { detail: { isSticky: isSticky, initialCheck: true } });
    ...
    _initObserver () {
        ...
        Sticky._observer = new window.IntersectionObserver(([entry]) => {
                ...
                const
                    stickyNav = entry.target
                        .parentElement; // entry.target is the `.sticky-trigger` and parentElement is the `.sticky-nav` element

                // check if `position` is actually `sticky` ... for example if we have `.sticky-nav-md`, it will be sticky only on small (md) devices
                if (isSticky &&
                    !stickyNav.classList
                    .contains("sticky-nav")) { // don't check `.sticky-nav` element because it's sticky regardless of window size

...
{
    public class InvLogWriter
    {
        private static InvLogWriter _instance;
        private static Queue<LogModel> _logQueue;
        private static readonly string logPath = ConfigurationManager.AppSettings["InvLogPath"] ?? "InvLogs";
        private static readonly string logFile = ConfigurationManager.AppSettings["InvLogFile"] ?? "InvLogsFile.log";
        ...
        private InvLogWriter()
        {
            _logQueue = new Queue<LogModel>();
        }

        public static InvLogWriter Instance
        {
            get
            {
                if (_instance != null) return _instance;
                _instance = new InvLogWriter();
                lock (_logQueue)
                {
                    _logQueue = new Queue<LogModel>();
                }

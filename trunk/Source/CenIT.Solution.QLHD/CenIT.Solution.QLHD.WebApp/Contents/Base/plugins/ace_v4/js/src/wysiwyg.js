...
const Default = {
    ...
    toolbar: [
        "font", null, "fontSize", null, "bold", "italic", "strikethrough", "underline", null, "insertunorderedlist",
        "insertorderedlist",
        "outdent", "indent", null, "justifyleft", "justifycenter", "justifyright", "justifyfull", null, "createLink",
        "unlink", null,
...
class Wysiwyg {
    ...
    initEditor () {
        ...
        toolbar.find(".dropdown-menu input[type=text]").on("click", function() { return false })
            .on("change",
                function() { $(this).closest(".dropdown-menu").siblings(".dropdown-toggle").dropdown("toggle") })
            .on("keydown",
                function(e) {
                    if (e.which === 27) {
                        this.value = "";
        ...
        toolbar.find("input[type=file]").prev().on("click",
            function(e) {
                // $(this).next().click()
            });
        ...
        toolbar.find("a[data-toggle=source]").on("click",
            function(e) {
                e.preventDefault();
        ...
        const $options = $.extend({},
            { activeToolbarClass: "active", toolbarSelector: toolbar },
    ...
    _createToolbarHtml () {
        ...
        for (const tb in toolbarButtons) {
            if (Object.prototype.hasOwnProperty.call(toolbarButtons, tb)) {
                ...
                const className = "className" in button
                    ? button.className
                    : "btn-sm btn-outline-secondary btn-h-light-dark btn-a-light-dark";
                switch (button.name) {
                case "font":
                    toolbarHtml += ` <a class="btn btn-sm ${className} dropdown-toggle" data-toggle="dropdown" title="${
                        button.title}"><i class="${button.icon}">${button.iconText || ""
                    ...
                    for (const font in button.values) {
                        if (Object.prototype.hasOwnProperty.call(button.values, font)) {
                            toolbarHtml += ` <div class="dropdown-item"><a data-edit="fontName ${button.values[font]
                                }" style="font-family:'${button.values[font]}'">${button.values[font]}</a></div> `;
                        }
                    }
                    toolbarHtml += " </div>";
                    break;
                ...
                case "fontSize":
                    ...
                    for (const size in button.values) {
                        if (Object.prototype.hasOwnProperty.call(button.values, size)) {
                            toolbarHtml += ` <div class="dropdown-item"><a data-edit="fontSize ${size}"><font size="${
                                size}">${button.values[size]}</font></a></div> `;
                        }
                    }
                    toolbarHtml += " </div> ";
                    break;
                ...
                case "createLink":
                    toolbarHtml += ` <div class="btn-group"> <a class="btn btn-sm ${className
                        } dropdown-toggle" data-toggle="dropdown" title="${button.title}"><i class="${button.icon}">${
                        button.iconText || ""}</i></a> `;
                    toolbarHtml +=
                        ` <div class="dropdown-menu py-1 px-3 brc-default-m3 border-2 radius-1 shadow-sm" style="min-width: 300px;">
						 <div class="input-group my-3">
							<input class="form-control" placeholder="${button.placeholder}" type="text" data-edit="${
                        button.name}" />
							<div class="input-group-append">
								<button class="btn btn-sm ${button.button_class}" type="button">${button.button_text
                        }</button>
							</div>
						 </div>
					</div> </div>`;
                ...
                case "insertImage":
                    ...
                    if (button.choose_file && "FileReader" in window) {
                        toolbarHtml +=
						   ...
						   <label class="text-center d-block mt-3 mb-0">
							<button class="btn btn-sm ${button.button_class} wysiwyg-choose-file" type="button">${
                    ...
                    toolbarHtml += `<div class="input-group my-3">
                        ...
                        button.name}" />
							<div class="input-group-append">
								<button class="btn btn-sm ${button.button_insert_class}" type="button">${button
                ...
                case "foreColor":
                case "backColor":
                    toolbarHtml += `<div class="mr-1px"><a class="btn btn-sm ${className
                        } dropdown-toggle" data-toggle="dropdown" title="${button.title}"><i class="${button.icon}">${
                        button.iconText || ""}</i></a> `;
                    toolbarHtml +=
                    ...
                    for (const color in button.values) {
                        if (Object.prototype.hasOwnProperty.call(button.values, color)) {
                            toolbarHtml +=
                                ...
                                button.name} ${button.values[color]
                                }" style="width:1.25rem;height:1.25rem;background-color:${button.values[color]
                                };"></a></div> `;
                        }
                    }
                ...
                case "viewSource":
                    toolbarHtml += ` <a class="btn btn-sm ${className}" data-toggle="source" title="${button.title
                        }"><i class="${button.icon}">${button.iconText || ""}</i></a> `;
                    break;
                default:
                    toolbarHtml += ` <a class="btn btn-sm ${className}" data-edit="${button.name}" title="${button.title
                        }"><i class="${button.icon}">${button.iconText || ""}</i></a> `;
                    break;
                }
            }
                ...
                $(currentImg).popover({
                    ...
                    content: function() {
                        return $(`<div class='btn-group'>
							  ...
							  <input type='number' class='form-control d-inline-block form-control-sm' data-action='resize' style='max-width: 96px;' placeholder='specify width' value='${
                            currentImg.width}' />
							  <hr class='my-2' />
							  <div class='btn-group'>

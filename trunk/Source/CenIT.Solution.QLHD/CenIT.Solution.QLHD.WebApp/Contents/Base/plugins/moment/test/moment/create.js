...
test("date with a format as an array",
    function(assert) {
        ...
        for (i = 0; i < tests.length; i++) {
            assert.equal(moment(tests[i]).format(),
...
test("string with array of formats",
    function(assert) {
        ...
        assert.equal(moment("02-11-1999", ["MM/DD/YYYY", "YYYY MM DD"]).format("MM DD YYYY"),
            "02 11 1999",
            "year last");
        assert.equal(moment("1999-02-11", ["MM/DD/YYYY", "YYYY MM DD"]).format("MM DD YYYY"),
            "02 11 1999",
            "year first");
        assert.equal(moment("02-11-1999", ["YYYY MM DD", "MM/DD/YYYY"]).format("MM DD YYYY"),
            "02 11 1999",
            "year last");
        ...
        assert.equal(moment("Thursday 8:30pm", ["dddd h:mma"]).format("YYYY MM DD dddd h:mma"),
...
test("parsing weekdays verifies the day",
    function(assert) {
        // string with format
        assert.ok(!moment("Wed 08-10-2017", "ddd MM-DD-YYYY").isValid(),
...
test("parsing weekday on utc dates verifies day acccording to utc time",
    function(assert) {
        assert.ok(moment.utc("Mon 03:59", "ddd HH:mm").isValid(), "Monday 03:59");
    });

test("parsing weekday on local dates verifies day acccording to local time",
    function(assert) {
...
test("parsing weekday on utc dates with specified offsets verifies day acccording to that offset",
    function(assert) {
        assert.ok(moment.utc("Mon 03:59 +12:00", "ddd HH:mm Z", true).isValid(), "Monday 03:59");
    });

test("parsing weekday on local dates with specified offsets verifies day acccording to that offset",
    function(assert) {

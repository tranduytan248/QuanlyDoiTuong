...
(function($) {
    ...
    Iconpicker.ICONSET = {
        ...
        transportation: $.iconset_transportation || Iconpicker.ICONSET_EMPTY
    };
    ...
    Iconpicker.prototype.bindEvents = function() {
        var op = this.options;
        var el = this;
        op.table.find(".btn-previous, .btn-next").off("click").on("click",
            function(e) {
                e.preventDefault();
        ...
        op.table.find(".btn-icon").off("click").on("click",
            function(e) {
                e.preventDefault();
                el.select($(this).val());
                if (op.inline === false) {
        ...
        op.table.find(".search-control").off("keyup").on("keyup",
            function() {
                el.changeList(1);
            });
    };
    ...
    Iconpicker.prototype.filterIcons = function() {
        ...
        if (op.iconsetVersion != "lastest" && typeof Iconpicker.ICONSET[op.iconset].allVersions != "undefined") {
            $.each(Iconpicker.ICONSET[op.iconset].allVersions,
                function(i, v) {
    ...
    Iconpicker.prototype.updateLabels = function(page) {
        ...
        var total_pages = this.totalPages();
        op.table.find(".page-count")
            .html(op.labelHeader.replace("{0}", (total_pages === 0) ? 0 : page).replace("{1}", total_pages));
        var offset = (page - 1) * this.totalIconsPerPage();
        var total = page * this.totalIconsPerPage();
        op.table.find(".icons-count").html(op.labelFooter.replace("{0}", total_icons ? offset + 1 : 0)
            .replace("{1}", (total < total_icons) ? total : total_icons).replace("{2}", total_icons));
    ...
    Iconpicker.prototype.setArrowNextIconClass = function(value) {
        this.options.arrowNextIconClass =
            this.removeAddClass(".btn-next > span", this.options.arrowNextIconClass, value);
    };

    Iconpicker.prototype.setArrowPrevIconClass = function(value) {
        this.options.arrowPrevIconClass =
            this.removeAddClass(".btn-previous > span", this.options.arrowPrevIconClass, value);
    };

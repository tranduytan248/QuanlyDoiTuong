...
test("explicit cloning",
    function(assert) {
        ...
        assert.notEqual(durationA.milliseconds(),
...
test("instatiation from serialized C# TimeSpan maxValue with + sign",
    function(assert) {
        var d = moment.duration("+10675199.02:48:05.4775808");
...
test("serialization to ISO 8601 duration strings",
    function(assert) {
        ...
        assert.equal(moment.duration({ y: -1, h: 1, s: -1 }).toISOString(),
            "-P1YT-59M-59S",
        ...
        assert.equal(moment.duration({ ms: 123456789 }).toISOString(),
            "PT34H17M36.789S",
...
test("toString acts as toISOString",
    function(assert) {
        assert.equal(moment.duration({ y: 1, M: 2, d: 3, h: 4, m: 5, s: 6 }).toString(),
            "P1Y2M3DT4H5M6S",
            "all fields");
...
test("bubble value up",
    function(assert) {
        ...
        assert.equal(moment.duration({ milliseconds: 61001 }).seconds(),
            1,
            "61001 milliseconds has 1 second left over");
        assert.equal(moment.duration({ milliseconds: 61001 }).minutes(),
            1,
...
test("asGetters",
    function(assert) {
        ...
        // milliseconds
        assert.equal(moment.duration(1, "millisecond").asYears().toFixed(13),
            0.0000000000317,

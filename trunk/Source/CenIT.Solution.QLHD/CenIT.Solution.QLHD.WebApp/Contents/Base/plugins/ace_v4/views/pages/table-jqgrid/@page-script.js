jQuery(function($) {
    ...
    $(".sidebar")
        .on("expand.ace.sidebar",
            function() {
                $(grid_box).hide();
            })
        .on("collapsed.ace.sidebar expanded.ace.sidebar",
            function() {
                $(grid_selector).jqGrid("setGridWidth", parent_column.width());
                $(grid_box).show();
            });
    ...
    $.jgrid.guiStyles.bootstrap4ace = {
        ...
        dialog: {
            ...
            closeButton:
                "mr-1 mt-n25 px-2 py-1 w-auto h-auto border-1 brc-h-warning-m1 bgc-h-warning-l1 text-danger radius-round",
    ...
    var _pageBtn =
        "btn w-4 h-4 px-0 mx-2px btn-outline-lightgrey btn-h-outline-primary btn-a-outline-primary radius-round bgc-white";
        ...
        loadComplete: function() {
            ...
            setTimeout(function() {
                    $(window).triggerHandler(
                        "resize.jqGrid"); //trigger window resize to make the grid get the correct size
    ...
    $(pager_selector).find(".navtable .action-btn").on("touchend",
        function() {
    ...
    function style_edit_form(form) {
        ...
        var buttons =
            form.parent().next().find(".EditButton .fm-button")
                .attr("href", "#"); // to disable for Bootstrap's "a:not([href])" style

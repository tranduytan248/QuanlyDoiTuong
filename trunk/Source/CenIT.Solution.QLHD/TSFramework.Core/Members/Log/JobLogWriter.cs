...
namespace TSFramework.Core.Members.Log
{
    public class JobLogWriter
    {
        ...
        private static readonly string logPath = ConfigurationManager.AppSettings["JobLogPath"] ?? "JobLogs";
        private static readonly string logFile = ConfigurationManager.AppSettings["JobLogFile"] ?? "JobLogsFile.log";
        ...
        public static JobLogWriter Instance
        {
            get
            {
        ...
        public void WriteToLog(string jobName, string message)
        {
            ...
            if (_logQueue.Count >= flushAtQty || CheckTimeToFlush()) FlushLogToFileAsync(jobName).ConfigureAwait(false);
        }
        ...
        {
            FlushLogToFileAsync(jobName).ConfigureAwait(false);
        }
        ...
        private static async Task FlushLogToFileAsync(string jobName)
        {
            await Task.Run(() =>
            {
                lock (fileLock)
                {
                    while (_logQueue.Count > 0)
                    {
                        ...
                        var path = Path.Combine(datedFolder, $"{entry.GetDate()}_{jobName}_{logFile}");

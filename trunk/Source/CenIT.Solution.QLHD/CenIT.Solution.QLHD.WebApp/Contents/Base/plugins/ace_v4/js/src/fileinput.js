...
class FileInput {
    constructor (element, config) {
        ...
        this.fileList = this.settings != undefined && this.settings.length > 0 ? this.settings.fileList : [];
        this.selectMethod = "";
    ...
    _applySettings () {
        ...
        this._wrap.querySelectorAll("*:not([type=file])")
            .forEach((el) => Util.remove(el)); // remove all except our input, good for when changing settings
...
<span class="ace-file-name ${this.settings.placeholderClass || ""}">
  ${this.settings.placeholderText}
            ...
            (!this._isDropStyle
                ? `<span class="ace-file-btn ml-auto ${this.settings.btnChooseClass || ""}">${this.settings
                .btnChooseText
                }</span>`
                : "") +
            "</div>";
        ...
        Util.after(this.element,
            `<div class="ace-file-container d-flex flex-column ${this.settings.container || ""}">${placeholder}</div>`);

        this.container = this.element.nextElementSibling;

        if (this.settings.reset !== false) {
            const remove = this.settings.reset.length > 0
                ? this.settings.reset
                : (!this._isDropStyle
            ...
            const btn = Util.append(this._wrap,
                `<a title="${this.settings.resetText || ""}" class="remove ${remove}" href="#">${this.settings.resetIcon
                }</a>`);

            if (this.settings.resetText && this._jQueryBS && window.jQuery.fn.tooltip)
                window.jQuery(btn).tooltip({ container: "body" });
    ...
    showFileList ($files, innerCall) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (this.settings.thumbnail)
                className += ` ${this.settings.thumbnail !== "small" ? "flex-column my-2px py-2" : "mx-1 py-1"
                    } align-items-center`;

            const label = Util.append(this.container,
                `<div class="${className}">
<span class="ace-file-icon align-self-center ${this.settings.iconClass || ""}">${fileIcon}</span>
<span class="ace-file-name ${this.settings.thumbnail && this.settings.thumbnail !== "small" ? "px-2" : "px-1"}">${
                filename}</span>` +
                (!this._isDropStyle
                    ? `<span class="ace-file-btn ml-auto ${this.settings.btnChangeClass || ""}">${this.settings
                    .btnChangeText}</span>`
                    : "") +
                "</div>");

            const type = (innerCall === true && this._hasFile && files[i] instanceof window.File)
                ? files[i].type.trim()
                : "";
            const canPreview = this.settings.previewImage !== false &&
                this._hasFileReader &&
                this.settings.thumbnail &&
                ((type.length > 0 && type.match("image")) ||
                (type.length === 0 &&
                    format ===
                    "image")); // the second one is for older Android's default browser which gives an empty text for file.type
            if (canPreview) {
                try {
                    this._previewImage(files[i], label)
                        .catch((result) => {
                            EventHandler.trigger(this.element,
                                Event.PREVIEW_FAILED,
                                { $_previewError: { filename: filename, code: result.code } });
                        });
                } catch (e) {
                    // for IE that doesn't support Promises
                    EventHandler.trigger(this.element,
                        Event.PREVIEW_FAILED,
                        { $_previewError: { filename: filename, code: PreviewError.FILE_LOAD_FAILED } });
                }
            }
        }
    }
    ...
    _enableFileDrop () {
        ...
        EventHandler.on(dropbox,
            ...
            (e) => {
                ...
                EventHandler.trigger(this.element,
                    Event.CHANGED,
                    { $_selectedFiles: { list: this.fileList, method: this.selectMethod } });
    ...
    _previewImage (file, label) {
        return new Promise((resolve, reject) => {
            const
                icon = label.querySelector(
                    ".ace-file-icon"); // it should be out of onload, otherwise all onloads may target the same $icon because of delays
            ...
            if (this._hasFile && file instanceof window.File) {
                ...
                if (file instanceof Object && Object.prototype.hasOwnProperty.call(file, "path")) {
                    getImage(file.path,
                        file); // file is a file name (path) --- this is used to pre-show user-selected image
                }
            }
        });
    }
    ...
    _checkFileList (files, dropped) {
        ...
        const event = EventHandler.trigger(this.element,
            Event.INVALID,
            {
                $_fileErrors: {

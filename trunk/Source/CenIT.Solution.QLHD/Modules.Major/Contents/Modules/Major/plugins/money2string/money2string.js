...
DocTienBangChu.prototype.doc = function (SoTien, coDonVi) {
    coDonVi = coDonVi == undefined || coDonVi == null ? false : true;
    var lan = 0;
    var i = 0;
    var so = 0;
    var KetQua = "";
    ...
    if (SoTien == 0) return coDonVi ? "Không đồng" : "Không";//"Không đồng !";
    if (SoTien > 0) {
        so = SoTien;
    }
    else {
    ...
    if (soAm) {
        return "Âm " + KetQua + (coDonVi ? " đồng" : "");//.substring(0, 1);//.toUpperCase();// + KetQua.substring(1);
    }
    else {
        return KetQua + (coDonVi ? " đồng" : "");//.substring(0, 1);//.toUpperCase();// + KetQua.substring(1);
    }
}
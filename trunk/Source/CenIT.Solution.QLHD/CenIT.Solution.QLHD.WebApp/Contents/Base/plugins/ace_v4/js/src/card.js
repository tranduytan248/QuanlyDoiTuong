...
class Card {
    constructor (element, config) {
        ...
        Util.reflow(this
            .element); // force reflow, so that if we instantly call 'close' etc, transition effect takes place
    ...
    close () {
        ...
        if (this.isFullscreen())
            Util.remove(Util.next(this.element, ".card-expanded-placeholder")); // remove the placeholder
            ...
            this.element.addEventListener("transitionend",
                (e) => {
                    if (
                        e.target !==
                            this.element)
                        return; // because transitionend might fire for children elements (like animated toolbar buttons)
                    _closeComplete();
                });
        }
    }
    ...
    toggle (type) {
        ...
        const action = (type && typeof type === "string" && type.match(/show|hide/)[0]) ||
            (body.offsetParent !== null ? "hide" : "show");
    ...
    expand (expand, animate) {
        ...
        if ($expand) {
            ...
            if (animate) {
                ...
                const transitionFn = (e) => {
                    if (
                        e.target !==
                            this.element)
                        return; // because transitionend might fire for children elements (like animated icons of toolbar)
                ...
                Util.after(this.element,
                    `<div class='card-expanded-placeholder' style='width: ${parseInt(rect.width)}px; height: ${
                    parseInt(rect.height)}px;'></div>`);
            ...
            animate = animate &&
                ...
                this.element.nextElementSibling.classList.contains("card-expanded-placeholder");
            if (animate) {
                this.isTransitioning = true;

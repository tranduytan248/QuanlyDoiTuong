$(function() {
    ...
    QUnit.test("should set basic options",
        function(assert) {
            assert.expect(4);

            var aside = $('<div id="aside-1" class="modal"><div class="modal-dialog"/></div>')
    ...
    QUnit.test("should be automatically hidden after 500ms",
        function(assert) {
            assert.expect(2);
            var done = assert.async();

            var aside = $('<div id="aside-123s" class="modal"><div class="modal-dialog"/></div>')
                .appendTo("#qunit-fixture");
            aside.aceAsideNC({
                autohide: 500
            }).aceAsideNC("show");
    ...
    QUnit.test("should hide aside when clicking outside of its area if it has `.modal-dismiss`",
        function(assert) {
            assert.expect(3);
            var done = assert.async(2);

            var aside = $(
                    '<div id="aside-1" class="modal ace-aside modal-nb modal-dismiss"><div class="modal-dialog"/></div>')
                .appendTo("#qunit-fixture");
            aside.on("shown.bs.modal",
                    function() {
    ...
    QUnit.test("should not hide aside when clicking inside its area if it has `.modal-dismiss`",
        function(assert) {
            assert.expect(1);
            var done = assert.async();
    ...
    QUnit.test("should return aside version",
        function(assert) {
            assert.expect(1);

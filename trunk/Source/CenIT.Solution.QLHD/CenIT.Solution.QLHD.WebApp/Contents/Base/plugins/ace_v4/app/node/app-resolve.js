...
module.exports = class AppResolve {
    ...
    static relativePath(path, to=null) {
        return relative(!to ? process.cwd() + "/" : to, path)
            .replace(/[\\\/]+/g, "/")
            .replace(/^([^../|./])/,
                "./$1"); //if relative path doesn't start with './' or '../' prepend it for node to be able to locate/access files correctly
    }
        ...
        if (pkgName) {
            if (pkgName == "app") {
                ...
                if (!
                    ...
                    pkgDir = this
                        .relativePath(
                            pkgName); //if no such node package, use pkgDir as the starting folder to perform the glob search
            }
        }

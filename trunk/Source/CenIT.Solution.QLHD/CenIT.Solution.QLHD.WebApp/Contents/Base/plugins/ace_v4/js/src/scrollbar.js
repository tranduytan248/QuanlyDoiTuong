...
class Scrollbar {
    ...
    enableScrollbars () {
        ...
        */
        ...
        if (this._config.ignore !== null) {
            if (this._config.ignore === "mobile" &&
                this._scrollbarInfo.width === 0 &&
                "ontouchstart" in window &&
                window.matchMedia("(max-width: 840px)").matches) return;
    ...
    _preferNativeScrolls () {
        if (this._scrollbarInfo.width === 0 ||
...
if (typeof window.jQuery !== "undefined") {
    ...
    $(window).on(Event.LOAD_DATA_API,
        () => {
            const scrollbars =
                ([].slice.call(document.querySelectorAll(Selector.ACE_SCROLL))).concat(
                    [].slice.call(document.querySelectorAll(Selector.DATA_ACE_SCROLL)));

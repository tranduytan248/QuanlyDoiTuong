...
class TabSwipe {
    ...
    _startPaneSwipe (ev, pane) {
        ...
        const paneTouchMoveCallback = (ev) => {
            ...
            if ((allowedDir === null || (allowedDir === "left")) &&
                ((!isRTL && moveX > 0) || (isRTL && moveX < 0))) newDir = 1;
            else if ((allowedDir === null || (allowedDir === "right")) &&
                ((!isRTL && moveX < 0) || (isRTL && moveX > 0))) newDir = -1;

            if (newDir !== 0 && newDir !== curDir) {
                if (
                    siblingPane !== null)
                    _reset(siblingPane); // undo previous direction for when we change swipe direction before a touchend
                curDir = newDir;
                ...
                siblingPane =
                    targetPane || (curDir === 1 ? curPane.nextElementSibling : curPane.previousElementSibling);
        ...
        const _reset = (sibling) => {
            ...
            sibling.style.transitionDuration =
                "0ms"; // so when we quickly and frequently swipe left/right without a `touchend`, the other pane moves away (disappears) instantly

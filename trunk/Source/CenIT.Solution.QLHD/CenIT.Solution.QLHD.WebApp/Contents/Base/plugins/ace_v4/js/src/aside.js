    ...
    static _HandleAside () {
        ...
        const onBeforeShow = (modal) => {
            if (modal.classList.contains("modal-nb")) {
                ...
                document.body.style.setProperty("--modal-padding",
                    (window.innerWidth - document.body.scrollWidth) + "px");
                ...
                if (backdropBg || isModalOff) {
                    setTimeout(function() {
                            ...
                            if (backdrops.length > 0) {
                                ...
                                if (isModalOff) {
                                    const modalOff = (isModalOff[1] && isModalOff[1].length > 0)
                                        ? `-${isModalOff[1]}`
                                        : "";
        ...
        const onAfterHide = (modal) => {
            if (document.querySelector(visibleModalSelector) === null)
                document.body.style.paddingRight = ""; // required for rare cases that body padding is still not cleared
            else
                document.body.classList
                    .add("modal-open"); // sometimes an aside is closed (so .modal-open is removed) but a .modal is still open (so we add .modal-open again)

...
class Sidebar {
    constructor (element, config) {
        ...
        Util.reflow(this
            ._sidebar); // force reflow, so that if we instantly call 'collapse' or 'expand', transition effect takes place
        ...
        this._desktopCollapsedClass = this._triggerArray.length > 0
            ? (this._triggerArray[0].getAttribute("data-toggle-class") || ClassName.DESKTOP_HIDE)
        ...
        EventHandler.on(this._inner,
            "focus",
            "input",
            (e) => {
                if (!this._collapsed) return;
                this._inner.classList.add("has-focus");
                EventHandler.one(e.delegateTarget,
                    "blur",
                    () => {
    ...
    expand (btn = null) {
         ...
         */
        if (Util.isReducedMotion() ||
            this._sidebar.classList.contains("no-transition") ||
            this._horizontal ||
            this._sidebar.offsetWidth === 0) this._toggleCompleted(); // call completion now
    ...
    collapse (btn = null) {
        ...
        if (btn !== null &&
            this._sidebar.classList.contains("expandable") &&
            this._sidebar.classList.contains("let-expanded") &&
            this._inner.contains(btn)) {
    ...
    show (btn = null) {
        ...
        if (scrollbarInfo.width === 0 &&
            this._sidebar.classList.contains("sidebar-fixed") &&
            (this._config.backdrop || this._sidebar.classList.contains("sidebar-push"))) {
    ...
    _addTransitionEvent () {
        ...
        this._sidebar.addEventListener("transitionend",
            (e) => {
        ...
        this._inner.addEventListener("transitionstart",
            (e) => {
        ...
        this._inner.addEventListener("transitionend",
            (e) => {
                ...
                if (this._inner.clientWidth < 140) {
                    ...
                    counter = 0;
                    // blur input element
                    if (document.activeElement.tagName === "INPUT" && this._inner.contains(document.activeElement))
        ...
        const touchMoveCallback = (ev) => {
            ...
            if (swipeDir === 0) {
                ...
                if (diffY > diffX) {
                    ...
                    this._inner.setAttribute("style", "touch-action: none;  transition: none !important;");
        ...
        const touchEndHandler = (e) => {
            ...
            if (swipeDir === 1 &&
                (// dismiss if moved by more than 100px or moved more than 40px in a short time (less than 300ms)
                    (!isRTL && (x1 - x2 > 100 || (x1 - x2 > 40 && t2 - t1 < 300))) ||
                        (isRTL && (x1 - x2 < -100 || (x1 - x2 < -40 && t2 - t1 < 300)))
                )
    ...
    enableSubmenuToggle () {
        ...
        EventHandler.on(this._sidebar,
            ...
            (ev) => {
                ...
                if ((this._collapsed && this._sidebar.classList.contains("hoverable")) ||
                    this._sidebar.classList.contains("sidebar-hover")) {
    ...
    _submenuScroll (subMenu, delay = 150) {
        if (subMenu &&
            !Util.isReducedMotion() &&
            "scrollBehavior" in document.documentElement.style &&
            this._config.subscroll &&
            this._sidebar.classList.contains("sidebar-fixed")) {
            if (this._sidebar.classList.contains(ClassName.HORIZONTAL)) {
                if (window.getComputedStyle(subMenu).position ===
                    "absolute") return; // no scroll for desktop horizontal menu (when it's not `.sidebar-hover`)
            }
        ...
        if (this._pullupCallback === null) {
            this._pullupCallback = (ev) => {
                if (ev.target !== ev.delegateTarget ||
                    ev.propertyName !== marginProp ||
                    !(this._collapsed || this._sidebar.classList.contains("sidebar-hover"))) return;

                const subMenu = ev.target;
                const navItem = subMenu.parentNode;

                const navText =
                    navItem.querySelector("* > .nav-link > .nav-text.fadeable"); // only first level nav-text items
                ...
                if (diff > 0) { // if submenu bottom is below window area
                    ...
                    if (this._collapsed) {
                        if (navTextHeight && diff > navTextHeight / 2 && navItem)
                            navItem.classList.add(
                                "submenu-pullup"); // this class makes the .sub-arrow's color white, to match submenu color
                    } else {

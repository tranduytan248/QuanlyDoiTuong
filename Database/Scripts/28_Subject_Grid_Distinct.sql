/* =============================================================================
   28. LUOI DOI TUONG GOP NHOM THEO CON NGUOI
   -----------------------------------------------------------------------------
   Sau khi cho phep moi can bo co ban ghi rieng cho cung mot CCCD (script 27),
   luoi se hien nhieu dong cho cung mot con nguoi. Script nay gop lai.

   GOP THEO: CCCD + Ho ten + Ngay sinh + Que quan.
   Trong moi nhom uu tien giu ban ghi cua CHINH nguoi dang dang nhap, vi do la
   ban ho co quyen sua/xoa. Khong co thi lay ban moi nhat.

   Bo sung cot RecordCount - so ban ghi cua cung con nguoi do, giao dien dung
   de bao cho nguoi dung biet con don vi khac cung dang quan ly ho so nay.

   GIU NGUYEN toan bo phan loc phan quyen (don vi + linh vuc) va sap xep.
   ============================================================================= */
...
BEGIN
    ...
    /* Gop nhom theo CCCD + Ho ten + Ngay sinh + Que quan.
       Ly do: mot nguoi co the duoc nhieu can bo cung nhap, moi nguoi mot ban
       ghi rieng. Tren luoi chi nen hien MOT dong cho moi con nguoi.

       Trong nhom, uu tien giu ban ghi cua chinh nguoi dang dang nhap - do la
       ban ho duoc sua/xoa. Khong co thi lay ban moi nhat. */
    Grouped AS
    (
        SELECT sc.*,
               ROW_NUMBER() OVER (
                   PARTITION BY sc.IdentityCardNumber, sc.FullName,
                                sc.DateOfBirth, sc.PlaceOfOrigin
                   ORDER BY CASE WHEN sc.CreatedBy = @UserName THEN 0 ELSE 1 END,
                            sc.CreatedDate DESC) AS RowInGroup,

               /* So ban ghi cua cung con nguoi nay - giao dien hien khi > 1 */
               COUNT(*) OVER (
                   PARTITION BY sc.IdentityCardNumber, sc.FullName,
                                sc.DateOfBirth, sc.PlaceOfOrigin) AS RecordCount
        FROM ScopedSubject AS sc
    ),
    Counted AS
    (
        SELECT g.*, COUNT(*) OVER () AS TotalRow
        FROM Grouped AS g
        WHERE g.RowInGroup = 1
    )
    SELECT
        c.SubjectId,
        ...
        (SELECT COUNT(1)
         FROM dbo.Major_SubjectViolations AS v
           ...
           /* Dem vi pham cua MOI ban ghi cung con nguoi, khong chi ban ghi dai
              dien. Neu chi dem ban dai dien thi so lan vi pham bi thieu phan do
              don vi khac ghi nhan. */
           WHERE s2.IdentityCardNumber = c.IdentityCardNumber
             AND ISNULL(s2.FullName, '') = ISNULL(c.FullName, '')
        ...
        /* So ban ghi cua cung con nguoi nay tren he thong. Lon hon 1 nghia la
           don vi khac cung dang quan ly ho so nay - giao dien se bao cho biet. */
        c.RecordCount,
